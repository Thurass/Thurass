...
**VM**. `HDESK` is retained in the repo only as a "why-not" reference.
...
- The full-resolution diagram set is in `diagrams/` (see §9).
...
- `PARALLEL-INPUT-FORWARDING.md` — **the corrected design** (read first).
- `PARALLEL-INPUT-FORWARDING.html` — self-contained visual companion (same content + diagrams).
- `VIRTUAL-DESKTOP-INPUT-FORWARDING.md` / `.html` — the HDESK "why-not" reference (superseded framing; still accurate about desktop objects).
- `SCHEMATIC.md` — **unrelated** (an iLOQ/TypeScript schematic; ignore for this task).

**Diagrams (`diagrams/`, hand-authored SVG):**
...
1. Read `PARALLEL-INPUT-FORWARDING.md` and skim the four corrected diagrams.
2. Record decisions D1–D7 (§6) at the top of a new `IMPLEMENTATION.md`.
